...
export default function ProgramDetailPage() {
    ...
    if (!program) {
        return (
            <ProtectedClient roles={['admin']}>
                <div className="min-h-screen flex flex-col items-center justify-center text-text-secondary gap-4">
            ...
            <div className="min-h-screen p-6 md:p-8 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-glass p-6 rounded-2xl border border-white/20 shadow-sm">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold text-(--color-text-primary) flex items-center gap-3">
                            {program.name}
                    ...
                    {TABS.map(tab => (
                        <button
                            ...
                            className={`px-6 py-3 font-bold text-sm transition-all border-b-2 whitespace-nowrap ${activeTab === tab
                                ...
                                : 'border-transparent text-text-secondary hover:text-(--color-text-primary) hover:bg-white/10'
                                } rounded-t-lg`}
                        >
                            {tab}
                        </button>
                ...
                <div className="bg-glass rounded-2xl p-8 border border-white/20 shadow-sm min-h-[400px]">
                    ...
                    {activeTab === 'OVERVIEW' && (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-6">
                                <h3 className="text-lg font-bold text-(--color-text-primary)">Operational Dashboard</h3>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    {/* Enrollment Stats */}
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30 hover:border-blue-200 transition-colors">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Students Enrolled</div>
                                    ...
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30 hover:border-orange-200 transition-colors">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Tutors Assigned</div>
                                    ...
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Duration</div>
                                    ...
                                    {/* Activity Stats */}
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Sessions Delivered</div>
                                        <div className="text-3xl font-bold text-green-600">{program.sessionsDelivered || 0}</div>
                                    </div>
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Upcoming</div>
                                        <div className="text-3xl font-bold text-purple-600">{program.upcomingSessions || 0}</div>
                                    </div>
                                    <div className="p-4 bg-white/50 rounded-xl border border-white/30">
                                        <div className="text-xs uppercase text-text-secondary font-bold">Hours Delivered</div>
                                        <div className="text-3xl font-bold text-(--color-text-primary)">{(program.hoursUsed || 0).toFixed(1)}h</div>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <h3 className="text-lg font-bold mb-4 text-(--color-text-primary)">Quick Actions</h3>
                    ...
                    {activeTab === 'STUDENTS' && (
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-(--color-text-primary)">Enrolled Students</h3>
                                <button
                            ...
                            <div className="bg-white/40 rounded-xl border border-white/20 overflow-hidden">
                                {program.students && program.students.length > 0 ? (
                                    <table className="w-full text-left">
                                        <thead className="bg-white/20 border-b border-white/10 text-xs uppercase text-text-secondary">
                                            <tr>
                                        ...
                                        <tbody>
                                            {program.students.map(student => (
                                                <tr key={student.id} className="border-b border-white/5 hover:bg-white/10">
                                                    <td className="p-4 font-medium text-(--color-text-primary)">{student.name}</td>
                                                    <td className="p-4 text-sm text-text-secondary">{student.enrolledAt ? format(new Date(student.enrolledAt), 'MMM d, yyyy') : 'N/A'}</td>
                    ...
                    {activeTab === 'STAFFING' && (
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-(--color-text-primary)">Assigned Tutors</h3>
                                <button
                                        ...
                                        <tbody>
                                            {program.tutors.map(tutor => (
                                                <tr key={tutor.id} className="border-b border-white/5 hover:bg-white/10">
                                                    <td className="p-4 font-medium text-(--color-text-primary)">{tutor.name}</td>
                                                    ...
                                                    <td className="p-4 text-center text-(--color-text-primary)">⭐ {tutor.rating}</td>
                                                    <td className="p-4 text-right">
                                                        <button className="text-red-500 text-sm hover:underline">Remove</button>
                                                    </td>
                                                </tr>
                    ...
                    {activeTab === 'COMPLIANCE' && (
                        <div className="space-y-8">
                            ...
                            <div className="p-8 text-center bg-white/30 rounded-2xl border border-dashed border-white/40">
                                ...
                                <h3 className="text-xl font-bold text-(--color-text-primary)">Compliance Dashboard</h3>
                                <p className="text-text-secondary mb-6">Trust scores, recording status, and attendance flags will appear here.</p>
                    ...
                    {!['OVERVIEW', 'STUDENTS', 'STAFFING', 'COMPLIANCE'].includes(activeTab) && (
                        <div className="space-y-6">
                            <h3 className="text-xl font-bold text-(--color-text-primary) capitalize">{activeTab.toLowerCase()} Details</h3>
                            ...
                            <dl className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {Object.entries(
                                    ...
                                    <div key={key} className="p-4 bg-white/30 rounded-xl border border-white/10">
                                        <dt className="text-xs uppercase text-text-secondary font-bold mb-1 tracking-wider">
                                            {key.replace(/([A-Z])/g, ' $1').trim()}
                                        </dt>
                                        <dd className="text-lg font-medium text-(--color-text-primary)">
