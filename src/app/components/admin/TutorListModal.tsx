...
export default function TutorListModal({
  isOpen,
  onClose,
}: TutorListModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setLoading((prev) => (prev ? prev : true)); // Avoid direct setState if possible, though this is similar
      api
        .get("/admin/tutors")
        .then((res) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      alert(
        "Failed to delete tutor. The feature might not be implemented on the backend.",
      );
    }
  };
  ...
  const handleToggleStatus = async (tutor: Tutor) => {
    ...
    try {
      // Optimistic update
      setTutors((prev) =>
        prev.map((t) => (t.id === tutor.id ? { ...t, status: newStatus } : t)),
      );
