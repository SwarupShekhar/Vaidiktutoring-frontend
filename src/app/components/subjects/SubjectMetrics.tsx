...
export default function SubjectMetrics() {
    return (
        <section className="w-full py-20 px-6 relative overflow-hidden bg-surface">
                    ...
                    <div>
                        ...
                        <div className="space-y-10">
                            <div className="flex gap-6 items-start">
                                <div className="text-4xl md:text-5xl font-extrabold text-sapphire">
                                    95%
                                </div>
                                <div>
                            ...
                            {[35, 45, 52, 65, 78, 92].map((height, i) => (
                                <div key={i} className="flex flex-col items-center gap-2 group/bar w-full relative z-20">
                                    <div className="relative w-full max-w-[40px] flex items-end h-[180px]">
                                        <div
                                            style={{ height: `${height}%` }}
                                            className={`w-full rounded-t-lg transition-all duration-1000 ease-out relative group-hover/bar:brightness-110 shadow-sm ${i === 5 ? 'bg-linear-to-t from-sapphire to-blue-400' :
                                                i >= 4 ? 'bg-sapphire' : 'bg-sapphire/40'
                                                }`}
                                        >
                                            {/* Tooltip */}
                                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-deep-navy text-white text-[10px] font-bold px-2 py-1 rounded opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                                                {height}%
                                            </div>
                                        </div>
                                    </div>
