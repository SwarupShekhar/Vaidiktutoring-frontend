        ...
        return (
            <div className="animate-pulse space-y-8 mt-12">
                <div className="grid grid-cols-4 gap-4 h-32">
                    {[1, 2, 3, 4].map(i => <div key={i} className="bg-surface rounded-2xl" />)}
                </div>
                    ...
                    {subject.stages.map((stage, idx) => {
                        ...
                        return (
                            <div key={stage.key} className="relative group flex flex-col h-full">
                                ...
                                {idx > 0 && (
                                    <div className="flex md:hidden justify-center items-center py-4 text-border">
                                        <div className="w-0.5 h-8 bg-linear-to-b from-border to-transparent" />
                                    </div>
                                )}

                                <div className={`flex flex-col bg-surface rounded-3xl p-8 border-2 ${borderColorClass} shadow-xl shadow-gray-200/50 dark:shadow-none hover:-translate-y-1 transition-transform duration-300 relative z-10 h-full`}>
                                    {/* Header */}
                                    <div className="mb-6">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="text-2xl">{iconEmoji}</span>
                                            <span className={`text-sm font-bold uppercase tracking-wider ${accentColorClass} dark:text-opacity-90 bg-background px-2 py-0.5 rounded-md border border-current opacity-80`}>
                                                Ages {stage.ageRange}
                                            </span>
                                        </div>
                                    ...
                                    <div className="space-y-4 mb-8 grow">
                                        <div>
                                            <span className="text-xs font-bold text-text-secondary uppercase block mb-1">Focus</span>
                                            <p className="font-medium text-(--color-text-primary) leading-snug">{stage.focus}</p>
                                        </div>
                                        <div>
                                            <span className="text-xs font-bold text-text-secondary uppercase block mb-1">Outcome</span>
                                            <p className="font-medium text-(--color-text-primary) leading-snug">{stage.outcome}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-3">
                                            {stage.curriculumFrameworks.map((cf, index) => (
                                                <div key={index} className="flex items-start gap-2">
                                                    ...
                                                    <div className="text-sm text-(--color-text-primary) font-bold">
                                ...
                                {/* Desktop Arrow */}
                                {idx < 2 && (
                                    <div className="hidden md:flex absolute -right-4 top-[15%] -translate-y-1/2 items-center justify-center z-20 text-sapphire">
