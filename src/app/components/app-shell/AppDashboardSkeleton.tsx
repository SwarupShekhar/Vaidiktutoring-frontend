      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-2.5">
            ...
            <div className="h-4 w-40 animate-pulse rounded bg-slate-200 dark:bg-white/6" />
          </div>
          <div className="h-9 w-32 animate-pulse rounded-full bg-slate-200 dark:bg-white/10" />
        </div>

        {/* hero next-class band */}
        <div className="h-40 w-full animate-pulse rounded-3xl bg-slate-200 dark:bg-white/6" />

        {/* stat / bento grid */}
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-44 animate-pulse rounded-2xl bg-slate-200 dark:bg-white/5" />
          ))}
        </div>
      </div>
    </div>
