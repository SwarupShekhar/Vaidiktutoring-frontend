    ...
    <section className="py-20 bg-background relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-12 left-[16%] right-[16%] h-0.5 bg-linear-to-r from-transparent via-sapphire/20 to-transparent z-0" />

          {STEPS.map((step, i) => (
            <FadeUpSection key={i} delay={i * 0.2} className="relative group">
              <div className="h-full p-8 rounded-4xl bg-surface border border-white/5 relative z-10 transition-all duration-300 hover:-translate-y-2 card-hover overflow-hidden">
                {/* Watermark Number */}
                ...
                <div className="w-14 h-14 rounded-2xl bg-sapphire/5 border border-sapphire/10 flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300 relative z-10 overflow-hidden p-2">
                  <Image
                    src={step.icon}
                    alt={step.title}
                    fill
