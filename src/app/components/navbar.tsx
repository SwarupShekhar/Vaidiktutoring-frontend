    ...
    <nav className="w-full sticky top-0 z-50 bg-white/70 dark:bg-black/70 backdrop-blur-xl border-b border-white/20 dark:border-white/5 transition-all duration-300">
      <div className="max-w-[1400px] mx-auto px-4 lg:px-8 xl:px-8 py-3 lg:py-4 flex items-center justify-between gap-2 xl:gap-3">
        ...
        {/* Center: Navigation - Desktop Only.
            min-w-0 + overflow-x-auto make THIS group the overflow victim when the
        ...
        <div className="flex items-center gap-2 lg:gap-3 shrink-0">
          ...
          <div className="relative group/country">
            ...
            >
              ...
              <span className="hidden 2xl:block text-text-secondary">{activeCurriculum.country}</span>
              <svg
          ...
          {(!user || user.role !== "tutor") && (
            <Link
              ...
              className="hidden sm:inline-flex items-center justify-center px-4 lg:px-5 py-2 rounded-full bg-primary text-white font-black text-xs tracking-wide hover:bg-sapphire hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-blue-500/20"
            >
              Book Demo
            </Link>
          )}
          ...
          {user ? (
            <button
              onClick={logout}
              className="hidden xl:block px-2 lg:px-3 py-2 text-xs font-bold text-text-secondary hover:text-red-500 transition-colors"
            >
              Logout
            </button>
          ) : (
            <Link
              href="/login"
              className="hidden xl:block px-2 lg:px-3 py-2 text-xs font-bold text-text-secondary hover:text-primary transition-colors"
            >
              Login
            </Link>
          )}
