...
export default function GlobalNotification() {
    ...
    return (
        <div className="fixed z-1000 pointer-events-none flex flex-col items-end gap-2 p-4 top-4 right-4 w-full max-w-sm">
            {notifications.map((notif) => {
                ...
                if (isLoud) {
                    return (
                        <div
                            key={notif.id}
                            className="fixed inset-0 flex items-center justify-center bg-black/60 pointer-events-auto backdrop-blur-sm p-4 z-10000 transition-opacity duration-300"
                        >
                            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 max-w-lg w-full border-2 border-purple-500 text-center relative overflow-hidden transform transition-all duration-300 scale-100">
                ...
                return (
                    <div
                        ...
                        className={`pointer-events-auto w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl border-l-4 p-4 flex gap-3 items-start backdrop-blur-md bg-opacity-95 transition-all duration-300 transform
