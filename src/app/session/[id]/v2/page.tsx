...
export default function SessionPage({ params }: SessionProps) {
    ...
    // V2 redesign: right-dock tab (chat / people)
    const [dockTab, setDockTab] = useState<'chat' | 'people'>('chat');
    ...
    return (
        <div className="v2-root w-screen h-screen flex flex-col bg-[#131218] text-[#f4f3f7] overflow-hidden">

            {/* ── ACCESS DENIED ─────────────────────────────────────────────── */}
            {accessDenied && (
                <div className="absolute inset-0 z-[60] bg-[#0d0c12]/97 flex items-center justify-center p-6">
                    <div className="w-full max-w-md bg-[#1a1922] border border-red-500/30 rounded-2xl p-8 text-white shadow-2xl text-center">
                        ...
                        <button
                            ...
                            className="w-full h-12 bg-[#7c6df6] hover:bg-[#6a5be0] text-white font-bold rounded-xl transition-all"
                        >
                            Back to dashboard
                        </button>
                    </div>
            ...
            {/* ── TUTOR PRE-JOIN PROMPT ─────────────────────────────────────── */}
            {!hasJoined && user?.role === 'tutor' && !isEnding && (
                <div className="absolute inset-0 z-50 bg-[#0d0c12]/95 flex items-center justify-center p-6">
                    <div className={`w-full max-w-md bg-[#1a1922] border rounded-2xl p-8 text-white shadow-2xl ${recordingAllowed ? 'border-[#7c6df6]/30' : 'border-white/10'}`}>
                        {recordingAllowed ? (
                            <>
                        ...
                        <button
                            ...
                            className="w-full h-12 bg-[#7c6df6] hover:bg-[#6a5be0] text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2"
                        >
                            Got it — Join Session →
                        </button>
                    </div>
            ...
            {/* ── TOP SESSION BAR ───────────────────────────────────────────── */}
            <header className="h-[52px] shrink-0 z-30 bg-[#1a1922] border-b border-white/8 flex items-center gap-3 px-3">
                {/* brand + subject */}
                <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-7 h-7 rounded-lg bg-[#7c6df6] grid place-items-center text-white font-black text-sm shrink-0">S</div>
                    <div className="min-w-0">
                        <div className="text-[13px] font-bold leading-tight truncate">{booking?.subject?.name || 'Session'}</div>
                        <div className="text-[10.5px] text-white/35 leading-tight truncate">{booking?.students?.first_name || 'Student'}&apos;s classroom</div>
                    </div>
                </div>

                <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-red-400 bg-red-500/12 px-2.5 py-1 rounded-full tracking-wide shrink-0">
                    <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> LIVE
                </span>

                {/* recording status */}
                <span
                    className={`hidden sm:inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full shrink-0 ${recordingAllowed ? 'text-red-300 bg-red-500/12' : 'text-white/45 bg-white/6 ring-1 ring-white/8'}`}
                    title={recordingAllowed ? 'The parent has consented — this session may be recorded for later review.' : 'This session is not being recorded.'}
                >
                    {recordingAllowed ? <><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> Recording</> : <><VideoOff size={12} /> Not recorded</>}
                </span>

                <div className="flex-1" />

                {/* reactions */}
                <div className="hidden md:flex gap-0.5 bg-white/5 rounded-xl p-1 border border-white/8 shrink-0">
                    {['👍', '🎉', '💡', '❓'].map(emoji => (
                        <button key={emoji} onClick={() => sendReaction(emoji)} className="w-8 h-8 grid place-items-center rounded-lg hover:bg-white/10 hover:scale-110 transition-all text-[15px] active:scale-95">
                            <span>{emoji}</span>
                        </button>
                    ))}
                    <button onClick={() => socket?.emit('whiteboard.triggerConfetti', { sessionId })} title="Confetti" className="w-8 h-8 grid place-items-center rounded-lg hover:bg-pink-500/20 text-pink-400 transition-all">
                        <Smile size={17} />
                    </button>
                </div>

                {/* timer */}
                <div className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 border tabular-nums shrink-0 transition-all ${timeRemaining <= 5 * 60 ? 'bg-red-600/90 border-red-500 text-white animate-pulse' : timeRemaining <= 10 * 60 ? 'bg-amber-500/90 border-amber-400 text-white' : 'bg-white/6 border-white/8 text-white'}`}>
                    <Timer size={13} className="opacity-60" />
                    <span className="font-bold text-[13px] tracking-tight">{formatTime(timeRemaining)}</span>
                    <span className="text-[10px] opacity-50 font-semibold">left</span>
                </div>

                {/* End */}
                <button onClick={handleEndSession} className="bg-red-500 hover:bg-red-600 text-white px-3.5 py-1.5 rounded-lg font-bold text-[12.5px] flex items-center gap-1.5 transition-all shrink-0">
                    <LogOut size={13} />
                    <span className="hidden sm:inline">End</span>
                </button>
            </header>

            {/* ── BODY: rail | board | dock ─────────────────────────────────── */}
            <div
                className="flex-1 min-h-0 grid transition-[grid-template-columns] duration-300"
                style={{
                    gridTemplateColumns: isScreenShareActive
                        ? `0px 1fr minmax(0, min(72vw, 960px))`
                        : `${user?.role === 'tutor' ? '56px' : '0px'} 1fr ${isPanelExpanded ? '360px' : '0px'}`,
                }}
            >
                {/* ---- TOOL RAIL (tutor only) ---- */}
                {user?.role === 'tutor' && (
                    <nav className="bg-[#1a1922] border-r border-white/8 flex flex-col items-center py-3 gap-1.5 overflow-hidden">
                        <span className="text-white/25 text-[8px] font-black tracking-[0.13em] uppercase mb-1">Present</span>
                        <label className="w-9 h-9 rounded-xl grid place-items-center cursor-pointer text-white/60 hover:bg-white/8 hover:text-white transition-all" title="Upload PDF / slides">
                            {uploadingSlides ? <span className="animate-spin text-sm">⏳</span> : <FileUp size={17} />}
                        ...
                        <button onClick={() => setShowAssetLibrary(!showAssetLibrary)} title="Content library" className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${showAssetLibrary ? 'bg-[#7c6df6]/18 text-[#a99bff] border border-[#7c6df6]/30' : 'text-white/60 hover:bg-white/8 hover:text-white'}`}>
                            <Library size={17} />
                        </button>

                        <div className="w-5 h-px bg-white/8 my-1.5" />
                        <span className="text-white/25 text-[8px] font-black tracking-[0.13em] uppercase mb-1">Board</span>
                        <button
                            onClick={() => {
                                const next = !isLaserMode;
                                setIsLaserMode(next);
                                if (excalidrawAPI) excalidrawAPI.updateScene({ appState: { activeTool: { type: next ? 'laser' : 'selection', customType: null } } });
                            }}
                            title="Laser pointer"
                            className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${isLaserMode ? 'bg-red-500/16 text-red-400 border border-red-500/32 animate-pulse' : 'text-white/60 hover:bg-white/8 hover:text-white'}`}
                        >
                            <div className={`w-3 h-3 rounded-full ${isLaserMode ? 'bg-red-400 shadow-[0_0_10px_2px_rgba(255,100,100,0.7)]' : 'border-2 border-current'}`} />
                        </button>
                        <button
                            onClick={() => {
                                socket?.emit('whiteboard.togglePenAccess', { sessionId, studentId: booking?.students?.id || 'student1', hasAccess: !hasPenAccess });
                                setHasPenAccess(!hasPenAccess);
                            }}
                            title={hasPenAccess ? 'Student can draw — tap to revoke' : 'Let the student draw'}
                            className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${hasPenAccess ? 'bg-emerald-500/16 text-emerald-400 border border-emerald-500/32' : 'text-white/60 hover:bg-white/8 hover:text-white'}`}
                        >
                            <PenTool size={16} />
                        </button>
                        <button onClick={() => setIsFocusMode(!isFocusMode)} title="Follow my view" className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${isFocusMode ? 'bg-blue-500/16 text-blue-400 border border-blue-500/32' : 'text-white/60 hover:bg-white/8 hover:text-white'}`}>
                            <Share2 size={16} className={isFocusMode ? 'animate-pulse' : ''} />
                        </button>
                        <button onClick={() => setShowClearConfirm(true)} title="Clear board" className="w-9 h-9 rounded-xl grid place-items-center text-white/45 hover:bg-red-500/12 hover:text-red-400 transition-all">
                            <Trash2 size={16} />
                        </button>

                        <div className="w-5 h-px bg-white/8 my-1.5" />
                        <span className="text-white/25 text-[8px] font-black tracking-[0.13em] uppercase mb-1">Check</span>
                        <button onClick={() => setShowPollModal(true)} title="Quick poll" className="w-9 h-9 rounded-xl grid place-items-center text-white/60 hover:bg-white/8 hover:text-white transition-all">
                            <BarChart size={16} />
                        </button>
                        <button onClick={() => setShowAttendance(!showAttendance)} title="Attendance" className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${showAttendance ? 'bg-[#7c6df6]/18 text-[#a99bff] border border-[#7c6df6]/30' : 'text-white/60 hover:bg-white/8 hover:text-white'}`}>
                        ...
                        <button
                            onClick={async () => {
                                try { await exportAndSharePdf(); toast.success('PDF shared with student!'); }
                                catch (error: any) { toast.error(`Failed to share PDF: ${error.response?.data?.message || error.message || 'Check connection'}`); }
                            }}
                            disabled={isExporting}
                            title="Save & share all slides as PDF"
                            className={`w-9 h-9 rounded-xl grid place-items-center transition-all ${isExporting ? 'bg-orange-500 text-white animate-pulse' : 'bg-emerald-600/16 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-600 hover:text-white'}`}
                        >
                            {isExporting ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <FileText size={16} />}
                        </button>
                    </nav>
                )}

                {/* ---- WHITEBOARD ---- */}
                <div className={`relative min-w-0 whiteboard-container ${isToolbarCollapsed ? 'whiteboard-collapsed' : ''} ${user?.role === 'student' || user?.role === 'parent' ? (hasPenAccess ? '' : 'pointer-events-none') : ''}`}>
                    {ExcalidrawComp ? (
                        <>
                            <ExcalidrawComp
                                ...
                                initialData={{ appState: { viewBackgroundColor: '#ffffff', currentItemFontFamily: 1, theme: 'light', zenModeEnabled: false, viewModeEnabled: user?.role === 'student' || user?.role === 'parent' ? !hasPenAccess : false } }}
                                UIOptions={{ canvasActions: { loadScene: false, saveToActiveFile: false, export: { saveFileToDisk: true }, saveAsImage: true, toggleTheme: false }, tools: { image: true }, dockedSidebarBreakpoint: 0 }}
                                libraryReturnUrl={undefined}
                                onLibraryChange={() => { }}
                            />

                            {/* toolbar collapse (tutor) */}
                            {user?.role === 'tutor' && (
                                <button
                                    onClick={() => setIsToolbarCollapsed(!isToolbarCollapsed)}
                                    className={`absolute left-2 top-14 z-[60] w-6 h-12 grid place-items-center bg-[#1a1922] border border-white/10 rounded-full text-white/40 hover:text-white transition-all shadow-xl ${isToolbarCollapsed ? 'opacity-100' : 'opacity-30 hover:opacity-100'}`}
                            ...
                            {/* view-only banner (student without pen) */}
                            {(user?.role === 'student' || user?.role === 'parent') && !hasPenAccess && (
                                <div className="absolute top-3 left-3 z-20 inline-flex items-center gap-2 bg-[#0d0c12]/80 backdrop-blur text-white text-[11px] font-medium px-3 py-1.5 rounded-full pointer-events-none">
                                    🔒 View only — ask your tutor to draw
                                </div>
                            )}

                            {/* following-tutor banner */}
                            {followingTutor && user?.role !== 'tutor' && (
                                <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 inline-flex items-center gap-2 bg-[#7c6df6] text-white text-[11px] font-bold tracking-wide px-3 py-1.5 rounded-full pointer-events-none animate-in fade-in slide-in-from-top-2 duration-300">
                                    <Share2 size={13} className="animate-pulse" /> Following tutor&apos;s view
                                </div>
                            )}
                            ...
                            <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
                                {Array.from(collaborators.entries()).map(([cid, collab]) => {
                                    ...
                                    return (
                                        <div key={`laser-${cid}`} className="absolute -ml-2 -mt-2 transition-all duration-75 ease-out" style={{ left: x, top: y }}>
                                            <div className="w-4 h-4 rounded-full bg-red-500 shadow-[0_0_20px_6px_rgba(255,0,0,0.6),0_0_8px_2px_white] animate-pulse" />
                                            {collab.username && (
                                                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase whitespace-nowrap">{collab.username}</div>
                                            )}
                                        </div>
                                    );
                                })}
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-white/40 bg-gray-50">
                            <div className="w-10 h-10 border-4 border-[#7c6df6] border-t-transparent rounded-full animate-spin mb-4" />
                            <p className="font-medium text-gray-500">Loading canvas…</p>
                        </div>
                    )}
                </div>

                {/* ---- RIGHT DOCK (video + tabs) ---- */}
                <aside className="bg-[#1a1922] border-l border-white/8 flex flex-col min-w-0 overflow-hidden" role="complementary" aria-label="Video and chat">
                    {/* video */}
                    <div className="p-2.5 shrink-0">
                        <div className="rounded-xl overflow-hidden border border-white/10 bg-black relative" style={{ height: isScreenShareActive ? 'calc(100vh - 220px)' : '218px' }}>
                            <div className="h-8 px-3 flex items-center justify-between bg-gradient-to-r from-[#4b3a86] to-[#3a2f6e] shrink-0">
                                <span className="text-[10px] font-black text-white/45 uppercase tracking-[0.15em]">
                                    {isScreenShareActive ? 'Screen share' : 'Video'}
                                </span>
                                <button
                                    onClick={() => setIsPanelExpanded(!isPanelExpanded)}
                                    className="text-white/70 hover:text-white hover:bg-white/15 rounded p-0.5 transition-colors"
                            ...
                            <div className="relative bg-black" style={{ height: 'calc(100% - 32px)' }}>
                                {hasJoined && dailyRoomUrl && dailyToken ? (
                                    <div
                                        ref={(el) => { dailyContainerRef.current = el; if (el && !isDailyContainerReady) setIsDailyContainerReady(true); }}
                                        className="absolute inset-0 w-full h-full border-0"
                                    />
                                ) : (
                                    <div className="absolute inset-0 grid place-items-center">
                                        <div className="text-center">
                                            <div className="animate-spin rounded-full h-9 w-9 border-4 border-[#7c6df6] border-t-transparent mx-auto mb-3" />
                                            <p className="text-white/45 text-xs">Connecting video…</p>
                                        </div>
                                    </div>
                                )}
                                {videoLoading && (
                                    <div className="absolute inset-0 grid place-items-center bg-black/50 backdrop-blur-sm z-10">
                                        <div className="animate-spin rounded-full h-8 w-8 border-2 border-[#7c6df6] border-t-transparent" />
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {/* student snapshot (tutor only) */}
                    {user?.role === 'tutor' && !snapshotHidden && (
                        <div className="mx-2.5 mb-2 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2.5 shrink-0">
                            <button onClick={() => setSnapshotExpanded(v => !v)} className="w-full flex items-center gap-2.5 text-left">
                                <div className="w-7 h-7 rounded-lg bg-[#7c6df6] grid place-items-center text-white font-black text-[13px] shrink-0">{studentData.name.charAt(0)}</div>
                                <div className="min-w-0 flex-1">
                                    <div className="text-[12.5px] font-bold leading-none truncate">{studentData.name}</div>
                                    <div className="text-[10.5px] text-emerald-400 leading-none mt-0.5">● Active now</div>
                                </div>
                                <ChevronDown size={14} className={`text-white/30 transition-transform ${snapshotExpanded ? 'rotate-180' : ''}`} />
                            </button>
                            {snapshotExpanded && (
                                <div className="mt-2.5">
                                    ...
                                    />
                                    <button onClick={() => { setSnapshotHidden(true); setSnapshotExpanded(false); }} className="mt-1.5 text-[10px] text-white/25 hover:text-white/50 transition-colors w-full text-right">Hide ×</button>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {/* tabs */}
                    <div className="flex gap-1 px-2.5 border-b border-white/8 shrink-0">
                        <button onClick={() => setDockTab('chat')} className={`flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-2.5 border-b-2 -mb-px transition-colors ${dockTab === 'chat' ? 'text-white border-[#7c6df6]' : 'text-white/40 border-transparent hover:text-white/70'}`}>
                            💬 Chat
                        </button>
                        <button onClick={() => setDockTab('people')} className={`flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-2.5 border-b-2 -mb-px transition-colors ${dockTab === 'people' ? 'text-white border-[#7c6df6]' : 'text-white/40 border-transparent hover:text-white/70'}`}>
                            👥 People <span className="text-white/30 font-bold">{sessionRoster.length + 1}</span>
                        </button>
                    </div>
                    ...
                    <div className="flex-1 min-h-0 overflow-hidden">
                        {dockTab === 'chat' ? (
                            <div className="h-full">
                                <SessionChat key={sessionId} sessionId={booking?.sessions?.[0]?.id || sessionId} socket={socket} />
                            </div>
                        ) : (
                            <div className="p-2.5 overflow-y-auto h-full">
                                {booking?.tutor && (
                                    <div className="flex items-center gap-2.5 py-2.5 border-b border-white/6">
                                        <div className="w-7 h-7 rounded-lg bg-[#7c6df6] grid place-items-center text-white font-black text-[12px]">{(booking.tutor.first_name || 'T').charAt(0)}</div>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-[12.5px] font-bold truncate">{booking.tutor.first_name} {booking.tutor.last_name || ''}</div>
                                            <div className="text-[10.5px] text-white/35">Tutor · host</div>
                                        </div>
                                        <span className="text-[9.5px] font-black text-emerald-400 bg-emerald-500/12 px-2 py-0.5 rounded-full uppercase tracking-wide">Live</span>
                                    </div>
                                )}
                                {sessionRoster.map((s) => (
                                    <div key={s.id} className="flex items-center gap-2.5 py-2.5 border-b border-white/6 last:border-0">
                                        <div className="w-7 h-7 rounded-lg bg-[#2f8f78] grid place-items-center text-white font-black text-[12px]">{(s.name || 'S').charAt(0)}</div>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-[12.5px] font-bold truncate">{s.name}</div>
                                            <div className="text-[10.5px] text-white/35">Student{hasPenAccess ? ' · pen granted' : ''}</div>
                                        </div>
                                        {hasPenAccess && <PenTool size={13} className="text-emerald-400" />}
                                    </div>
                                ))}
                                {sessionRoster.length === 0 && !booking?.tutor && (
                                    <p className="text-white/30 text-xs text-center py-8">Waiting for participants…</p>
                                )}
                            </div>
                        )}
                    </div>
            ...
            {/* collapsed-dock reopen tab */}
            {!isPanelExpanded && !isScreenShareActive && (
                <button
                    onClick={() => setIsPanelExpanded(true)}
                    className="absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-[#7c6df6] hover:bg-[#6a5be0] text-white rounded-l-xl py-3 px-1.5 shadow-lg"
            ...
            {/* ── FILMSTRIP (slides) ────────────────────────────────────────── */}
            {slides.length > 0 && (
                <div className="h-[64px] shrink-0 z-10 bg-[#1a1922] border-t border-white/8 flex items-center gap-2 px-3 overflow-x-auto no-scrollbar">
                    <span className="text-white/25 text-[9px] font-black tracking-[0.13em] uppercase shrink-0 mr-1">Slides</span>
                    {user?.role === 'tutor' && (
                        <button onClick={() => switchSlide(currentSlideIndex - 1)} disabled={currentSlideIndex === 0} className="w-8 h-8 rounded-lg bg-white/8 hover:bg-white/15 grid place-items-center text-white disabled:opacity-25 shrink-0" title="Previous slide">
                            <ChevronLeft size={15} />
                        </button>
                    )}
                    {slides.map((slide, index) => (
                        <button key={index} onClick={() => switchSlide(index)} className={`shrink-0 w-[74px] h-[44px] rounded-md overflow-hidden border-2 transition-all ${index === currentSlideIndex ? 'border-[#7c6df6] scale-105' : 'border-white/12 hover:border-white/35 opacity-60 hover:opacity-100'}`} title={`Slide ${index + 1}`}>
                            <img src={slide} alt={`Slide ${index + 1}`} className="w-full h-full object-cover" />
                        </button>
                    ))}
                    {user?.role === 'tutor' && (
                        <button onClick={() => switchSlide(currentSlideIndex + 1)} disabled={currentSlideIndex === slides.length - 1} className="w-8 h-8 rounded-lg bg-white/8 hover:bg-white/15 grid place-items-center text-white disabled:opacity-25 shrink-0" title="Next slide">
                            <ChevronRight size={15} />
                        </button>
                    )}
                    <span className="ml-auto text-white/30 text-[10px] font-semibold shrink-0 tabular-nums">{currentSlideIndex + 1} / {slides.length}</span>
                    {user?.role === 'tutor' && (
                        <button onClick={() => { if (window.confirm('Clear all slides?')) { setSlides([]); socket?.emit('whiteboard.syncSlides', { sessionId, slides: [] }); } }} className="ml-2 text-[9px] font-black text-red-500/50 hover:text-red-400 uppercase tracking-widest shrink-0">Clear</button>
                    )}
                </div>
            )}

            {/* ── ASSET LIBRARY DRAWER (tutor) ──────────────────────────────── */}
            {user?.role === 'tutor' && showAssetLibrary && (
                <div className="absolute left-[64px] top-[60px] bottom-4 w-72 hidden md:flex bg-white/[0.98] backdrop-blur-xl border border-[#7c6df6]/20 shadow-2xl rounded-2xl p-4 overflow-y-auto flex-col z-40">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="font-bold text-lg text-[#3c3489] leading-none">Session library</h2>
                        <button onClick={() => setShowAssetLibrary(false)} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
                    </div>
                    <div className="flex bg-gray-100 p-1 rounded-xl mb-4 shrink-0 overflow-x-auto no-scrollbar">
                        {(['vault', 'assets', 'manipulatives'] as const).map((tab) => (
                            <button key={tab} onClick={() => setLibraryTab(tab)} className={`flex-1 py-1.5 px-3 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap ${libraryTab === tab ? 'bg-white text-[#7c6df6] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                                {tab === 'vault' ? 'Materials' : tab === 'assets' ? 'Session' : 'Tools'}
                            </button>
                        ))}
                    </div>
                    <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
                        {libraryTab === 'vault' ? (
                            <div className="flex flex-col h-full -mx-4 -mb-4 border-t border-gray-100">
                                <VaultSidebar onSelectAsset={handleVaultAssetSelect} selectedAssetId={selectedVaultAsset?.id} currentSubject={booking?.subject?.name} />
                            </div>
                        ) : libraryTab === 'assets' ? (
                            <div className="grid grid-cols-1 gap-3">
                                <div className="flex items-center justify-between mb-1 ml-1 pr-1">
                                    ...
                                    <button onClick={() => (document.getElementById('asset-upload-input-v2') as HTMLInputElement)?.click()} className="text-[9px] font-black text-[#7c6df6] hover:text-[#3c3489] uppercase tracking-widest flex items-center gap-1"><FileUp size={10} /> Upload</button>
                                    <input
                                        id="asset-upload-input-v2" type="file" accept=".pdf,image/*" className="hidden"
                                        onChange={(e) => {
                                            const file = e.target.files?.[0];
                                            if (!file) return;
                                            const name = file.name.toLowerCase();
                                            if (name.endsWith('.ppt') || name.endsWith('.pptx')) { toast.error('PowerPoint files cannot be rendered directly. Please export your slides as a PDF first, then upload the PDF.', { duration: 8000 }); return; }
                                            if (name.endsWith('.pdf')) { renderPdfLocally(file); return; }
                                            const reader = new FileReader();
                                            reader.onload = (event) => {
                                                const url = event.target?.result as string;
                                                setSlides(prev => { const next = [...prev, url]; socket?.emit('whiteboard.syncSlides', { sessionId, slides: next }); return next; });
                                ...
                                {slides.map((url, index) => (
                                    <div key={index} className="border rounded-xl cursor-pointer overflow-hidden hover:ring-2 hover:ring-[#7c6df6]/50 transition-all bg-gray-50 flex flex-col group" onClick={() => importImageToExcalidraw(url)}>
                                        ...
                                        <div className="p-2 text-[9px] font-black uppercase tracking-tight text-center text-gray-500 bg-white border-t">{`Slide ${index + 1}`}</div>
                                    </div>
                                ))}
                                {slides.length === 0 && (
                                    <div className="text-center py-12 px-4 border-2 border-dashed border-gray-100 rounded-3xl flex flex-col items-center gap-3">
                                        <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-300"><FileUp size={24} /></div>
                                        <p className="text-[10px] font-bold text-gray-400 leading-normal max-w-[140px] mx-auto uppercase tracking-wider">No assets uploaded yet. Upload an image or PDF slide to add it here.</p>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {Object.entries(MANIPULATIVES_DATA).map(([grade, items]) => (
                                    <div key={grade} className="border-b border-gray-100 last:border-0 pb-2">
                                        <button onClick={() => setExpandedSections(prev => ({ ...prev, [grade]: !prev[grade] }))} className="w-full flex items-center justify-between py-2 text-left hover:bg-gray-50 rounded-lg px-2 transition-colors">
                                            <span className="text-[11px] font-black text-[#3c3489] uppercase tracking-widest">{grade === 'k-1' ? 'Grades K–1' : grade === '2-3' ? 'Grades 2–3' : grade === '4-6' ? 'Grades 4–6' : grade === 'math' ? 'Advanced Math' : grade === 'physics' ? 'Physics' : grade === 'chemistry' ? 'Chemistry' : grade}</span>
                                            {expandedSections[grade] ? <ChevronDown size={14} className="text-[#7c6df6]" /> : <ChevronRight size={14} className="text-[#7c6df6]" />}
                                        </button>
                                        {expandedSections[grade] && (
                                            <div className="grid grid-cols-2 gap-2 mt-2 px-1">
                                                {items.map(item => (
                                                    <div key={item.id} className="relative group">
                                                        <button onClick={() => insertManipulative(item)} title={item.label} className="w-full flex flex-col items-center justify-center p-3 bg-gray-50 border border-gray-100 rounded-xl hover:border-[#7c6df6]/40 hover:bg-white hover:shadow-lg transition-all active:scale-95">
                                                            <span className="text-2xl mb-1 group-hover:scale-110 transition-transform duration-300">{item.thumbnail}</span>
                                                            <span className="text-[9px] font-black text-gray-700 text-center leading-tight group-hover:text-[#7c6df6] transition-colors">{item.label}</span>
                                                        </button>
                                                        {item.id.includes('dice') && (
                                                            <button
                                                                onClick={(e) => {
                                                                    ...
                                                                    insertManipulative({ ...item, elements: [item.elements[0], ...DICE_FACES[roll as keyof typeof DICE_FACES]], label: `Dice Rolled (${roll})` });
                                                                    toast.success(`🎲 Rolled a ${roll}!`);
                                                                }}
                                                                className="absolute top-1 right-1 bg-[#7c6df6] text-white text-[8px] font-black px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10"
                                                            >ROLL</button>
                                                        )}
                                                    </div>
                                                ))}
            ...
            {/* ── ATTENTION PANEL (tutor) ───────────────────────────────────── */}
            {user?.role === 'tutor' && showAttentionPanel && (
                <div className="absolute bottom-24 left-[72px] z-40 w-[380px] pointer-events-auto">
                    <div className="h-[450px] animate-in slide-in-from-left-4 duration-500">
                        <AttentionFrameworkPanel sessionId={sessionId} studentId={booking?.students?.id || ''} tutorId={booking?.tutor?.id || ''} socket={socket} />
                    </div>
                </div>
            )}

            {/* ── ATTENDANCE ────────────────────────────────────────────────── */}
            <AttendanceTracker isOpen={showAttendance} onClose={() => setShowAttendance(false)} sessionId={sessionId} students={sessionRoster} onSave={saveAttendance} />

            {/* ── END-SESSION NOTE MODAL (tutor) ────────────────────────────── */}
            {showNoteModal && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-[#1a1922] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 border border-white/10">
                        <div className="p-6">
                            <h2 className="text-2xl font-black text-white mb-2">Wrap up session</h2>
                            <p className="text-sm text-white/60 mb-6">Please leave a quick note for the parents about {booking?.students?.first_name || 'the student'}&apos;s progress today.</p>
                            <div className="relative">
                                <textarea value={sessionNote} onChange={(e) => setSessionNote(e.target.value.slice(0, 500))} placeholder="e.g. Alice did great with fractions today! We should focus more on long division next time…" className="w-full h-40 p-4 bg-white/5 border-2 border-white/10 rounded-2xl focus:border-[#7c6df6] focus:ring-0 transition-all text-sm text-white placeholder:text-white/20 resize-none" />
                                <div className="absolute bottom-3 right-3 text-[10px] font-bold text-white/40">{500 - sessionNote.length} characters remaining</div>
                            </div>
                        </div>
                        <div className="bg-white/5 p-4 flex gap-3 border-t border-white/10">
                            <button onClick={handleEndSession} className="flex-1 px-4 py-3 text-sm font-bold text-white/40 hover:text-white/70 transition-colors">Not now, just end</button>
                            <button
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        try { await exportAndSharePdf(); toast.success('Annotated PDF shared with student!'); }
                                        catch (error: any) { toast.error(`Failed to share PDF: ${error.response?.data?.message || error.message || 'Check connection'}`); }
                                        await handleEndSession();
                                    } catch { toast.error('Failed to end session properly'); setSubmittingNote(false); }
                                }}
                                disabled={submittingNote}
                                className="flex-[2] px-6 py-3 bg-[#7c6df6] hover:bg-[#6a5be0] text-white rounded-xl font-bold shadow-lg disabled:opacity-50 transition-all"
                            >
            ...
            {/* ── WRAP-UP CAPSULE ───────────────────────────────────────────── */}
            {showWrapUp && (
                <div className="absolute top-20 left-1/2 -translate-x-1/2 z-50 animate-bounce cursor-pointer" onClick={() => setShowWrapUp(false)}>
                    <div className="bg-orange-500 border-2 border-orange-400 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3">
                        ...
                        <p className="font-bold text-sm">10 minutes remaining! Time to wrap up and ask final questions.</p>
                        <button onClick={(e) => { e.stopPropagation(); setShowWrapUp(false); }} className="ml-2 w-6 h-6 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white font-bold">✕</button>
                    </div>
                </div>
            )}
            ...
            <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden">
                {reactions.map(r => (
                    <div key={r.id} className="absolute bottom-0 animate-float-up" style={{ left: `${r.x}%`, fontSize: '64px', animationDelay: `${r.delay || 0}s` }}>{r.emoji}</div>
                ))}
            </div>

            {/* ── QUICK CHECK / POLL MODAL (tutor) ──────────────────────────── */}
            {showPollModal && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-[#1a1922] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 border border-white/10">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <button onClick={() => setShowPollModal(false)} className="text-white/40 hover:text-white/70"><X size={20} /></button>
                            </div>
                            <label className="block text-[10px] font-black text-[#a99bff] uppercase tracking-widest mb-1.5 ml-1">Question (max 120 chars)</label>
                            <input type="text" maxLength={120} value={pollQuestion} onChange={(e) => setPollQuestion(e.target.value)} placeholder="e.g. What is the square root of 64?" className="w-full p-4 bg-white/5 border-2 border-white/10 rounded-2xl focus:border-[#7c6df6] focus:ring-0 transition-all text-sm text-white placeholder:text-white/20 mb-4" />
                            <label className="block text-[10px] font-black text-[#a99bff] uppercase tracking-widest mb-1.5 ml-1">Options (min 2)</label>
                            <div className="space-y-2 mb-6">
                                {['A', 'B', 'C', 'D'].map((label, idx) => (
                                    <div key={label} className="relative">
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-black text-[#a99bff]/60">{label}</div>
                                        <input type="text" value={pollOptions[idx]} onChange={(e) => { const n = [...pollOptions]; n[idx] = e.target.value; setPollOptions(n); }} placeholder={`Option ${label}`} className="w-full pl-10 pr-4 py-3 bg-white/5 border-2 border-white/10 rounded-xl focus:border-[#7c6df6] focus:ring-0 transition-all text-sm text-white placeholder:text-white/20" />
                                    </div>
                                ))}
                            </div>
                            <button
                                onClick={() => {
                                    ...
                                    if (!pollQuestion.trim() || validOptions.length < 2) { toast.error('Enter a question and at least 2 options'); return; }
                                    socket?.emit('poll:launch', { sessionId, question: pollQuestion, options: validOptions, userId: user?.id });
                                    setShowPollModal(false); setPollQuestion(''); setPollOptions(['', '', '', '']);
                                }}
                                className="w-full py-4 bg-[#7c6df6] hover:bg-[#6a5be0] text-white rounded-2xl font-bold shadow-lg transition-all active:scale-95"
                            >🚀 Launch poll</button>
                        </div>
                    </div>
                </div>
            ...
            {/* ── TUTOR ACTIVE POLL RESULTS ─────────────────────────────────── */}
            {user?.role === 'tutor' && activePoll && (
                <div className="absolute top-[64px] left-1/2 -translate-x-1/2 z-40 w-full max-w-sm px-4">
                    <div className="bg-[#1a1922]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl p-5 animate-in slide-in-from-top-4 duration-300">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" /><span className="text-[10px] font-black text-white/50 uppercase tracking-widest">Live poll active</span></div>
                        ...
                        <div className="space-y-2 mb-6">
                            {activePoll.options.map((opt, idx) => {
                                const count = pollResults?.results[idx] || 0; const total = pollResults?.totalResponses || 0; const pct = total > 0 ? Math.round((count / total) * 100) : 0;
                                return (
                                    <div key={idx} className="relative h-8 bg-white/5 rounded-lg overflow-hidden border border-white/10">
                                        <div className="absolute inset-y-0 left-0 bg-[#7c6df6]/25 border-r border-[#7c6df6]/40 transition-all duration-500 ease-out" style={{ width: `${pct}%` }} />
                                        <div className="absolute inset-0 px-3 flex items-center justify-between text-[11px] font-medium"><span className="text-white truncate pr-4">{opt}</span><span className="text-[#a99bff] font-bold">{count} ({pct}%)</span></div>
                                    </div>
                                );
                            })}
                        </div>
                        <button onClick={() => socket?.emit('poll:close', { sessionId, userId: user?.id })} className="w-full py-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-xl text-xs font-black uppercase tracking-widest transition-all">Close poll & show results</button>
                    </div>
                </div>
            )}
            ...
            {user?.role === 'student' && activePoll && (
                <div className="fixed inset-0 z-40 bg-[#26215c]/50 backdrop-blur-lg flex items-center justify-center p-6 pb-24">
                    <div className="bg-[#1a1922] rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/10">
                        <div className="p-8 md:p-10 text-center">
                            <div className="w-16 h-16 bg-[#7c6df6]/12 rounded-2xl flex items-center justify-center mx-auto mb-6"><BarChart className="text-[#a99bff]" size={32} /></div>
                            <h2 className="text-sm font-black text-[#a99bff] uppercase tracking-widest mb-2">Quick check!</h2>
                            <h3 className="text-2xl font-black text-white mb-8 leading-tight">{activePoll.question}</h3>
                            {studentSelection === null ? (
                                <div className="grid grid-cols-1 gap-3">
                                    {activePoll.options.map((opt, idx) => (
                                        <button key={idx} onClick={() => { setStudentSelection(idx); socket?.emit('poll:respond', { sessionId, userId: user.id, optionIndex: idx }); }} className="p-5 bg-white/5 hover:bg-[#7c6df6] text-white border-2 border-white/10 hover:border-[#a99bff] rounded-2xl text-left font-bold transition-all active:scale-[0.98] group">
                                            <div className="flex items-center gap-4"><div className="w-8 h-8 rounded-lg bg-white/10 group-hover:bg-[#8f82ff] flex items-center justify-center text-sm font-black text-[#a99bff] group-hover:text-white transition-colors">{['A', 'B', 'C', 'D'][idx]}</div>{opt}</div>
                                        </button>
                                    ))}
                                </div>
                            ) : (
                                <div className="py-10">
                                    <div className="w-12 h-12 bg-emerald-500/10 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-4"><Smile size={24} /></div>
            ...
            {finalPollResults && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-6">
                    <div className="bg-[#1a1922] rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/10">
                        <div className="p-8 text-center">
                            <div className="w-16 h-16 bg-emerald-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6"><span className="text-3xl">📊</span></div>
                            <h2 className="text-xs font-black text-emerald-500 uppercase tracking-widest mb-2">Poll results</h2>
                            <h3 className="text-xl font-bold text-white mb-6">{finalPollResults.question}</h3>
                            <div className="space-y-2 text-left">
                                {finalPollResults.options.map((opt, idx) => {
                                    const count = finalPollResults.results[idx] || 0; const total = finalPollResults.totalResponses || 0; const pct = total > 0 ? Math.round((count / total) * 100) : 0;
                                    return (
                                        <div key={idx} className="relative h-10 bg-white/5 rounded-xl overflow-hidden border border-white/10">
                                            <div className="absolute inset-y-0 left-0 bg-emerald-500/20 border-r border-emerald-500/30 transition-all duration-700 ease-out" style={{ width: `${pct}%` }} />
                                            <div className="absolute inset-0 px-4 flex items-center justify-between text-xs font-bold"><span className="text-white truncate pr-4">{opt}</span><span className="text-emerald-400">{count} ({pct}%)</span></div>
                                        </div>
                                    );
                                })}
                            </div>
                            <div className="mt-8 pt-6 border-t border-white/5"><p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Closed by tutor</p></div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {incomingSticker && (
                <div className="fixed inset-0 z-50 pointer-events-none flex items-center justify-center">
                    <div className="bg-white/10 backdrop-blur-md rounded-full p-12 lg:p-20 relative animate-sticker-reveal">
                        ...
                        <img src={`/stickers/${STICKER_FILE_MAP[incomingSticker.type] || incomingSticker.type + '.png'}`} alt="Sticker reward" className="w-48 h-48 lg:w-64 lg:h-64 object-contain relative z-10" />
                        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-center whitespace-nowrap"><h2 className="text-3xl lg:text-5xl font-black text-white italic tracking-tighter drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">MARVELOUS! ⭐</h2></div>
                    </div>
                </div>
            )}
            ...
            {showClearConfirm && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-[#1a1922] border border-white/10 rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6"><Trash2 className="text-red-500" size={32} /></div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button onClick={() => setShowClearConfirm(false)} className="py-3 px-6 bg-white/5 hover:bg-white/10 text-white font-bold rounded-xl transition-all">Cancel</button>
                            <button
                                onClick={() => {
                                    if (!excalidrawAPI) return;
                                    ...
                                    setShowClearConfirm(false); toast.success('Board cleared');
                                }}
                                className="py-3 px-6 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-lg transition-all"
                            >Clear all</button>
                        </div>
                    </div>
                </div>
            ...
            <style>{`
                ...
                .animate-sticker-reveal { animation: sticker-reveal 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
                .no-scrollbar::-webkit-scrollbar { display: none; }
                .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
                .whiteboard-collapsed .excalidraw .App-menu_top, .whiteboard-collapsed .excalidraw .island {
                    opacity: 0.1; pointer-events: none; transform: translateX(-100px); transition: all 0.3s ease;
                }
                .whiteboard-collapsed .excalidraw .App-menu_top:hover, .whiteboard-collapsed .excalidraw .island:hover {
